# import pygame
# from settings import SCREEN_WIDTH, SCREEN_HEIGHT, PLAYER_HP, PLAYER_TOUCH_DAMAGE, WHITE, HEALTH_BAR_BG_COLOR, HEALTH_BAR_BORDER_COLOR, ZOMBIE_SPEED, ZOMBIE_HP
...
#
#     def spawn_zombie(self, speed=None, hp=None):
...
#         elif level == 3:
#             self.spawn_zombie(speed=max(1, ZOMBIE_SPEED - 1), hp=ZOMBIE_HP * 10)
#
...
class Game:
    def __init__(self):
        ...
        self.font = pygame.font.SysFont('Arial', 24)
        self.defeated_font = pygame.font.SysFont('Arial', 85)
        self.victory_font = pygame.font.SysFont('Arial', 85)
        self.defeated = False
        self.victory = False
        self.defeated_time = None
        self.configure_level(1)
    ...
    def draw(self):
        ...
        if self.defeated:
            ...
            self.screen.blit(text, rect)
        if self.victory:
            text = self.victory_font.render('Vitoria', True, (0, 255, 0))
            rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            self.screen.blit(text, rect)